					...
					{Object.values(
						_.mapValues(
							_.groupBy(
								...
								({ path }) => path.afterChange
							),
							(gitFileChanges) =>
								gitFileChanges.reduce((acc, cur) => ({
									...acc,
									numLinesAdded:
										acc.numLinesAdded ?? 0 + (cur.numLinesAdded ?? 0),
									numLinesDeleted:
										acc.numLinesDeleted ?? 0 + (cur.numLinesDeleted ?? 0),
								}))
						)
					)
						.sort(
							(a, b) =>
								Math.abs((b.numLinesAdded ?? 0) + (b.numLinesDeleted ?? 0)) -
								Math.abs((a.numLinesAdded ?? 0) + (a.numLinesDeleted ?? 0))
						)
